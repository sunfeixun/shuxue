(function(){
	...
	p.go = function(){
		new father.title('你能找出所有的蓝色/红色/黄色/紫色/绿色的图形吗？',p);
		...
		let datas = [['triangle','yellow'],['rectangle','purple'],['triangle','red'],['circle','green'],['rectangle','red'],
					['square','blue'],['square','purple'],['triangle','blue'],['triangle','green'],['circle','blue'],
					['triangle','purple'],['circle','yellow'],['circle','red'],['rectangle','green']];
		...
		let cls = [['blue','蓝色'],['red','红色'],['yellow','黄色'],['purple','紫色'],['green','绿色']];
		...
		dragTargets.set({y:335}).sumAttr('x',160,230);
	...
	function judge(e){
		...
		for(let i=0;i<dragTargets.array.length;i++){
			...
			if(father.publicFunction.pointInRect(e.localX,e.localY,curTarg.getTransformedBounds())){
				 if(curTarg.color===e.target.color){
