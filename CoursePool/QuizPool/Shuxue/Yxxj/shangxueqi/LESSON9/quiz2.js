(function() {
	...
	p.go = function(){
		...
		let elements = [
			{type:sprite.obj1,clone:4,instance:'topobj',center:true},
			{type:sprite.obj1,clone:3,instance:'bottomobj'},
			{type:new father.cubeNumber(4),attr:{x:'center',y:480},instance:'ques'},
			{type:new father.Cube,attr:{x:'center-=150',y:250},instance:'ans1'},
			{type:new father.cubeNumber(2),attr:{x:'center+=150',y:250},instance:'ans2'},
		...
		new father.Line([[a1.x-10,a1.top()-10,a2.x,a2.bottom()+10],[a1.x+10,a1.top()-10,a3.x,a3.bottom()+10]]).addTo(p);
		topObj.set({y:a1.bottom()+50}).sumAttr('x',481,106);
		...
		bottomObj.con.y = dynamicAns.y - 80;
	...
	function fresh(){
		...
		let v = dynamicAns.value(father.publicFunction.random(3));
	...
	function judge(e){
		if(father.publicFunction.pointInRect(e.localX,e.localY,targRect.getTransformedBounds())){
			if(e.target.value() === 4 - dynamicAns.value()){
