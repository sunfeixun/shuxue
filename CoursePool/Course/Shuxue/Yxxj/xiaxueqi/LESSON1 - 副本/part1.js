(function() {
	...
	p.go = function() {
		...
		//问号对应的形状和文本
		let queTxtCon;
		let qt1 = new father.text('正方形',25).set({textAlign:'center',textBaseline:'middle'});
		let qts = [
			qt1,
			qt1.clone().set({text:'长方形'}),
			qt1.clone().set({text:'圆形'}),
			qt1.clone().set({text:'三角形'})
		];


		let sps = [
			new myObject.shape('square',{diameter:56}).lineColor('black'),
			new myObject.shape('rectangle',{width:69,height:37}).lineColor('black'),
			new myObject.shape('circle',{diameter:56}).lineColor('black'),
			new myObject.shape('triangle',{width:56,height:48}).lineColor('black')
		]
		...
		for(i=1;i<=4;i++){
			...
			i===4? _ques.custom.tween1.call(allQueShow,[true],null,'-=0.5'):_ques.custom.tween1.call(allQueShow,null,null,'-=0.5');
	...
	p.reset = function() {
		...
		shapeOrder = [
				{an:'case1',txts:['扁的','薄的','直的','尖尖的']},
				{an:'case1',txts:['扁的','薄的','直的','尖尖的']},
				{an:'case1',txts:['没有尖','一直转','薄的','弯弯的']},
				{an:'case2',txts:['尖尖的','直的','薄的']},
			];
	...
	function onAned(e) {
		switch(girl.currentAnimation){
			case 'wait11':case 'wait21':case 'wait13':case 'wait23':
				showPop1();
				break;
			case 'wait12':case 'wait22':case 'wait14':
				showPop2();
				break;
		}

		if(girl.currentAnimation==='wait14'||girl.currentAnimation==='wait23'){
